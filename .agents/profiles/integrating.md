...
- In integration mode only worry about what is currently contained in feature files and not so much about actively tracking the things *you* do
- In integration mode you should worry about resolving feature file conflicts (where the most likely solution is likely going to just be adding both changes, though I leave you the discretion to determine what's best)
...
Similar to the feature files, you just worry about resolving conflicts in parameter files.
...
In general, I prefer to move faster, so if there are conflicts I'd probably stick to as far away from number 3 as possible, but also sometimes the entire point of a task is to debug, so the feature should be focused on stage 4: DEBUGGINg.
